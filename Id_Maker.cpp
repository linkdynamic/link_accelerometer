...
ICACHE_FLASH_ATTR Id_MakerClass::Id_MakerClass() {
  useRNG = true;
  lastYield = 0;
}

ICACHE_FLASH_ATTR int Id_MakerClass::randomBitRaw(void) {
...
ICACHE_FLASH_ATTR int Id_MakerClass::randomBitRaw2(void) {
...
ICACHE_FLASH_ATTR int Id_MakerClass::randomBit(void) {
  // Software whiten bits using Von Neumann algorithm
  //
  // von Neumann, John (1951). "Various techniques used in connection
  // with random digits". National Bureau of Standards Applied Math Series
...
ICACHE_FLASH_ATTR char Id_MakerClass::randomByte(void) {
  char result = 0;
  uint8_t i;
...
ICACHE_FLASH_ATTR int Id_MakerClass::rand() {
  int result = 0;
  uint8_t i;
...
ICACHE_FLASH_ATTR long Id_MakerClass::random() {
  long result = 0;
  uint8_t i;
...
ICACHE_FLASH_ATTR long Id_MakerClass::random(long howBig) {
  long randomValue;
  long topBit;
  long bitPosition;
...
ICACHE_FLASH_ATTR long Id_MakerClass::random(long howSmall, long howBig) {
  if (howSmall >= howBig) return howSmall;
  long diff = howBig - howSmall;
  return Id_MakerClass::random(diff) + howSmall;
}

ICACHE_FLASH_ATTR void Id_MakerClass::memfill(char* location, int size) {
  for (;size--;) *location++ = randomByte();
}

ICACHE_FLASH_ATTR void Id_MakerClass::mac(uint8_t* macLocation) {
  memfill((char*)macLocation,6);
}

ICACHE_FLASH_ATTR void Id_MakerClass::uuid(uint8_t* uuidLocation) {
...
ICACHE_FLASH_ATTR String Id_MakerClass::uuidToString(uint8_t* uuidLocation) {
  String string = "";
  int i;
  for (i=0; i<16; i++) {
    if (i==4) string += "-";
